...
func Test_AddVolumeNode_Positive_NewVolumeNewNodeWithFalseAttached(t *testing.T) {
	...
	reportAsAttachedVolumesMap, _ := asw.GetVolumesToReportAttachedForNode(logger, nodeName)
	if len(reportAsAttachedVolumesMap) > 0 {
...
func Test_AddVolumeNode_Positive_NewVolumeTwoNodesWithFalseAttached(t *testing.T) {
	...
	reportedVolumes, _ := asw.GetVolumesToReportAttachedForNode(logger, node2Name)
...
func Test_RemoveVolumeFromReportAsAttached(t *testing.T) {
	...
	if removed {
		t.Fatalf("RemoveVolumeFromReportAsAttached succeeded. Should wait for SetNodeStatusUpdateFinished")
	}
	if !updated {
		t.Fatalf("update hook is not called")
	}

	volumes, g1 := asw.GetVolumesToReportAttachedForNode(logger, nodeName)
	if !slices.Equal(volumes, []v1.AttachedVolume{{Name: vol2}}) {
		t.Fatalf("reportAsAttachedVolumes Expected: <vol2> Actual: <%v>", volumes)
	}

	// remove after get generation
	...
	asw.SetNodeStatusUpdateFinished(logger, nodeName, g1)
	// Should success after async node update
	removed = asw.RemoveVolumeFromReportAsAttached(vol1, nodeName)
	if !removed {
		t.Fatalf("RemoveVolumeFromReportAsAttached failed. SetNodeStatusUpdateFinished not effective")
	}
	// vol2 should still fail
	removed = asw.RemoveVolumeFromReportAsAttached(vol2, nodeName)
	if removed {
		t.Fatalf("RemoveVolumeFromReportAsAttached succeeded. Should wait for SetNodeStatusUpdateFinished")
	}
...
// Test_SetNodeStatusUpdateFinishedError expects the map nodesToUpdateStatusFor
// to be empty if the SetNodeStatusUpdateFinished is called on a node that
// does not exist in the actual state of the world
func Test_SetNodeStatusUpdateFinishedError(t *testing.T) {
	// Arrange
	volumePluginMgr, _ := volumetesting.GetTestVolumePluginMgr(t)
	asw := NewActualStateOfWorld(volumePluginMgr)
	...
	// Act
	logger, _ := ktesting.NewTestContext(t)
	asw.SetNodeStatusUpdateFinished(logger, nodeName, 999)
...
// Test_SetNodeStatusUpdateFinished expects volumesToReportAsAttached to be properly updated if
// SetNodeStatusUpdateFinished is called on a node that exists in the actual state of the world
func Test_SetNodeStatusUpdateFinished(t *testing.T) {
	// Arrange
	volumePluginMgr, _ := volumetesting.GetTestVolumePluginMgr(t)
	...
	logger, _ := ktesting.NewTestContext(t)
	asw.SetNodeStatusUpdateFinished(logger, nodeName, 1)

	volumes := asw.GetNodesToUpdateStatusFor()[nodeName].volumesToReportAsAttached
	if !maps.Equal(volumes, map[v1.UniqueVolumeName]int64{"volume-2": 2, "volume-3": -1}) {
		t.Fatalf("expect volumes-2 and volumes-3 to be reported as attached, got %v", volumes)
	}

	asw.SetNodeStatusUpdateFinished(logger, nodeName, 2)

	volumes = asw.GetNodesToUpdateStatusFor()[nodeName].volumesToReportAsAttached
	if !maps.Equal(volumes, map[v1.UniqueVolumeName]int64{"volume-3": -1}) {
